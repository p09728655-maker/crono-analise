...
export function serieDoPeriodo(conferencias, maquina) {
  ...
  if (n < MIN_MEDICOES_TENDENCIA) {
    ...
    return {
      ...
      pct: 0, pctBruto: 0, r2: 0, r2Minimo: 1, nEfetivo: 0, pecas: [], misturaPecas: false,
    };
  }
  ...
  let direcao = 'estavel';
  if (limpa && Math.abs(pctLimpo) >= RUIDO_PCT && limpa.r2 >= r2Minimo) {
...
export function lerTendenciaPeriodo(serie) {
  const { n, direcao, pct, pctBruto, misturaPecas, reta } = serie;
  const variacao = Math.abs(Math.round(pct));
  const brutoTxt = reta
    ? `${Math.round(reta.inicio)} → ${Math.round(reta.fim)} pç/h no gráfico`
    : '';
  ...
  if (direcao === 'subindo') {
    return {
      rotulo: 'Ritmo subindo',
      tom: 'ok',
      mostrarPct: true,
      frase: `Descontado o efeito da peça, o ritmo subiu ${variacao}% no período (${brutoTxt}). `
  ...
  if (direcao === 'caindo') {
    return {
      ...
      tom: 'atencao',
      mostrarPct: true,
      frase: `Descontado o efeito da peça, o ritmo caiu ${variacao}% no período (${brutoTxt}). `
        + 'Olhar broca, abastecimento e ajustes antes que vire perda de capacidade.',
    };
  }

  /* A reta bruta inclina, mas o que a empurra e' a peca medida em cada
     data, nao o tempo. E' o erro mais caro deste relatorio: concluir que a
     maquina piorou quando o que mudou foi o que ela estava furando. */
  if (misturaPecas && Math.abs(pctBruto) >= RUIDO_PCT) {
    return {
      rotulo: 'Efeito da peça',
      tom: 'neutro',
      mostrarPct: true,
      frase: `A linha do gráfico varia ${Math.abs(Math.round(pctBruto))}% (${brutoTxt}), mas isso acompanha `
        + 'a peça medida em cada data, não o tempo — peça diferente rende diferente na mesma máquina. '
  ...
  return {
    ...
    tom: 'ok',
    mostrarPct: false,
    frase: `Em ${n} medições o ritmo não subiu nem caiu de forma consistente no período: `
      + 'o número desta máquina serve como referência de capacidade.',
  };
}
